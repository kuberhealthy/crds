...
- The controller code for listening for and acting on events is generated in `internal/controller/`.
...
```sh
make docker-build docker-push IMG=<some-registry>/crds:tag
```
...
```sh
make deploy IMG=<some-registry>/crds:tag
```
...
```sh
make build-installer IMG=<some-registry>/crds:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/crds/<tag or branch>/dist/install.yaml
```
